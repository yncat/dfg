...
test("render currentRoomInfo for waiting status", () => {
  ...
  const memberList = screen.getByText("メンバー: cat、dog、rabbit、lion、tiger、monkey");
  const stat = screen.getByText("現在の状況: catさんがゲームを開始するまで待機しています。");
  const lastRes = screen.getByText("前回の結果: catが大富豪。rabbitが富豪。tiger、lionが平民。dogが貧民。monkeyが大貧民。");
